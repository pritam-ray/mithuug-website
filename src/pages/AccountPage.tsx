        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <aside className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm p-6 space-y-2">
              ...
              <Link to="/account/orders" className="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-stone-100 text-stone-600 hover:text-stone-900 transition-colors">
                <Package className="w-5 h-5" />
                <span>Orders</span>
              </Link>
              <Link to="/account/wishlist" className="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-stone-100 text-stone-600 hover:text-stone-900 transition-colors">
                <Heart className="w-5 h-5" />
                <span>Wishlist</span>
              </Link>
              <Link to="/account/settings" className="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-stone-100 text-stone-600 hover:text-stone-900 transition-colors">
                <Settings className="w-5 h-5" />
                <span>Settings</span>
              </Link>
              <button onClick={handleSignOut} className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-red-50 text-red-600 transition-colors">
          ...
          <main className="lg:col-span-3 space-y-8">
            ...
            <div className="bg-white rounded-lg shadow-sm p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link to="/account/orders" className="text-stone-600 hover:text-stone-900">View All</Link>
              </div>
              {orders.length === 0 ? (
                ...
                <div className="space-y-4">
                  {orders.map((order) => (
                    <div key={order.id} className="flex items-center justify-between p-4 border border-stone-200 rounded-lg hover:border-stone-400 transition-colors">
                      <div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link to="/account/wishlist" className="text-stone-600 hover:text-stone-900">View All</Link>
              </div>
              {wishlist.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {wishlist.slice(0, 4).map((item) => (
                    <Link key={item.id} to={`/product/${item.product_id}`} className="group">
                      ...
                      <p className="text-sm text-stone-900 line-clamp-1">{item.product?.name}</p>
                    </Link>
                  ))}
                </div>
              )}
